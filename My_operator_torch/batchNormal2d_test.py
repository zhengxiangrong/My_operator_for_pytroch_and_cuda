...
class model1(nn.Module):
    def __init__(self):
        ...
        self.conv2d = nn.Conv2d(3,4,(2,2),bias=False)
        # self.conv2d.weight = nn.Parameter(weight)
        self.normal = nn.BatchNorm2d(4,momentum=1)
...
class model2(nn.Module):
    ...
    def forward(self, input):
        ...
        normal_out = self.conv2d(input)
        # print("model2 self.conv2d.weight.detach().numpy()", self.conv2d.weight.detach().numpy())
...
import numpy as np
for i in range(100):
    input = torch.rand([5, 3, 3, 4])
    y_out=torch.rand([5,4,2,3],dtype=torch.float32)
